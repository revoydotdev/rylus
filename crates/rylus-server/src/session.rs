...
enum EncodeCommand {
    /// New pixel data to encode. Keyframe requests and quality changes ride
    /// along with the frame instead of being separate commands: the channel
    /// has capacity 1 and drops when the encoder is busy, and a dropped
    /// standalone RequestKeyframe would strand the client on a frozen stream
    /// exactly when it asked for recovery.
    Frame {
        owned: rylus_core::pixel::OwnedPixelData,
