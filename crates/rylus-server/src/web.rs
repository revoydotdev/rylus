...
fn boxed_response(status: StatusCode, content_type: &str, body: &str) -> Response<BoxBody<Bytes, Infallible>> {
    Response::builder()
        .status(status)
        .header("content-type", content_type)
...
async fn serve(
    ...
    match (req.method(), req.uri().path()) {
        (&Method::GET, "/settings") => {
            return Ok(response_from_str(SETTINGS_HTML, "text/html; charset=utf-8").map(|r| r.boxed()));
        }
...
async fn handle_post_config(
    ...
    Ok(boxed_response(StatusCode::OK, "application/json", r#"{"ok":true}"#))
}
