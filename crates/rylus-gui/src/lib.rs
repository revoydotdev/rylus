...
use rylus_core::config::{write_config, Config, ThemeType};
...
pub fn run(config: &Config, log_receiver: mpsc::Receiver<String>, mut rylus: Box<dyn RylusServer>) {
    ...
    let mut wind = Window::default()
        .with_size(660, 600)
        .center_screen()
        .with_label(&format!("Rylus - {}", env!("CARGO_PKG_VERSION")));
    wind.set_xclass("rylus");
    ...
    input_access_code.set_tooltip(
        ...
        any kind of encryption and it is advised to only run Rylus inside trusted networks! Do \
        NOT reuse any of your passwords! If left blank, no code is required to access Rylus \
        remotely.",
    );
    ...
    check_auto_start.set_tooltip("Start Rylus server immediately on program start.");
            ...
            if !is_server_running {
                {
                    ...
                    let bind_addr: IpAddr = match input_bind_addr.value().parse() {
                        Ok(addr) => addr,
                        Err(e) => {
                            fltk::dialog::alert_default(&format!("Invalid bind address: {}", e));
                            return Ok(());
                        }
                    };
                    let web_port: u16 = match input_port.value().parse() {
                        Ok(port) => port,
                        Err(e) => {
                            fltk::dialog::alert_default(&format!("Invalid port: {}", e));
                            return Ok(());
                        }
                    };
                ...
                if !rylus.start(&config, Box::new(|message| match message {
                    UInputInaccessible => awake_callback(move || {
                        ...
                        let mut pop_up = Window::default()
                            .with_size(w, h)
                            .center_screen()
                            .with_label("Rylus - UInput inaccessible!");
                        pop_up.set_xclass("rylus");
...
fn custom_input_area_window_handle_events(
    win: &mut Window,
    sender: std::sync::mpsc::Sender<rylus_core::protocol::CustomInputAreas>,
) {
    #[derive(Debug)]
    enum MouseFlags {
        All,
            ...
            fltk::enums::Event::KeyDown => match app::event_key() {
                fltk::enums::Key::Enter => {
                    fn relative_rect(win: &Window, workspace: &Rect) -> Rect {
                        ...
                        let mut rect = rylus_core::protocol::Rect {
